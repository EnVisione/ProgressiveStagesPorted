...
import com.enviouse.progressivestagesported.common.util.StageUtil;
...
public class LockRegistry {
    ...
    private final Map<Identifier, StageId> itemLocks = new ConcurrentHashMap<>();

    // Item Tag -> Required Stage
    private final Map<Identifier, StageId> itemTagLocks = new ConcurrentHashMap<>();
    ...
    private final Map<Identifier, StageId> recipeLocks = new ConcurrentHashMap<>();

    // Recipe Tag -> Required Stage
    private final Map<Identifier, StageId> recipeTagLocks = new ConcurrentHashMap<>();

    // Recipe Item Locks: Output Item ID -> Required Stage (lock recipe by output item, not item itself)
    private final Map<Identifier, StageId> recipeItemLocks = new ConcurrentHashMap<>();

    // Block ID -> Required Stage
    private final Map<Identifier, StageId> blockLocks = new ConcurrentHashMap<>();

    // Block Tag -> Required Stage
    private final Map<Identifier, StageId> blockTagLocks = new ConcurrentHashMap<>();
    ...
    private final Map<Identifier, StageId> fluidLocks = new ConcurrentHashMap<>();

    // v1.4: Fluid Tag -> Required Stage (hide from EMI/JEI)
    private final Map<Identifier, StageId> fluidTagLocks = new ConcurrentHashMap<>();
    ...
    private final Map<Identifier, StageId> dimensionLocks = new ConcurrentHashMap<>();
    ...
    private final Map<Identifier, StageId> entityLocks = new ConcurrentHashMap<>();

    // Entity Tag -> Required Stage
    private final Map<Identifier, StageId> entityTagLocks = new ConcurrentHashMap<>();
    ...
    private final Set<Identifier> unlockedItems = ConcurrentHashMap.newKeySet();

    // v1.4: Global whitelist of blocks that are ALWAYS unlocked (bypass all block lock checks)
    private final Set<Identifier> unlockedBlocks = ConcurrentHashMap.newKeySet();

    // v1.3: Global whitelist of entities that are ALWAYS unlocked (bypass all entity lock checks)
    private final Set<Identifier> unlockedEntities = ConcurrentHashMap.newKeySet();

    // v1.4: Global whitelist of fluids that are ALWAYS unlocked (bypass mod locks in EMI/JEI)
    private final Set<Identifier> unlockedFluids = ConcurrentHashMap.newKeySet();
    ...
    public void registerStage(StageDefinition stage) {
        ...
        for (String itemId : locks.getUnlockedItems()) {
            Identifier rl = parseIdentifier(itemId);
            if (rl != null) {
        ...
        for (String blockId : locks.getUnlockedBlocks()) {
            Identifier rl = parseIdentifier(blockId);
            if (rl != null) {
        ...
        for (String entityId : locks.getUnlockedEntities()) {
            Identifier rl = parseIdentifier(entityId);
            if (rl != null) {
        ...
        for (String fluidId : locks.getUnlockedFluids()) {
            Identifier rl = parseIdentifier(fluidId);
            if (rl != null) {
    ...
    private void registerItemTagLock(String tagId, StageId stageId) {
        ...
        Identifier rl = parseIdentifier(cleanTag);
        if (rl != null) {
    ...
    private void registerRecipeLock(String recipeId, StageId stageId) {
        Identifier rl = parseIdentifier(recipeId);
        if (rl != null) {
    ...
    private void registerDimensionLock(String dimId, StageId stageId) {
        Identifier rl = parseIdentifier(dimId);
        if (rl != null) {
    ...
    private Optional<StageId> computeRequiredStageForItem(Item item) {
        ...
        // Check item tags
        for (Map.Entry<Identifier, StageId> entry : itemTagLocks.entrySet()) {
     ...
     */
    public Optional<StageId> getRequiredStageForRecipe(Identifier recipeId) {
        if (recipeId == null) {
            return Optional.empty();
        }
     ...
     */
    public Map<Identifier, StageId> getAllRecipeItemLocks() {
    ...
    private boolean matchesExemptionList(Item item, List<String> exemptions) {
        ...
        for (String entry : exemptions) {
            if (entry.startsWith("#")) {
                ...
                try {
                    Identifier tagLoc = Identifier.parse(tagStr);
    ...
    public Optional<StageId> getRequiredStageForBlock(Block block) {
        ...
        Identifier itemId = BuiltInRegistries.ITEM.getKey(block.asItem());
        ...
        // Check block tags
        for (Map.Entry<Identifier, StageId> entry : blockTagLocks.entrySet()) {
     ...
     */
    public Optional<StageId> getRequiredStageForDimension(Identifier dimensionId) {
        if (dimensionId == null) {
            return Optional.empty();
        }
    ...
    public Optional<StageId> getRequiredStageForEntity(net.minecraft.world.entity.EntityType<?> entityType) {
        ...
        // Check entity tags
        for (Map.Entry<Identifier, StageId> entry : entityTagLocks.entrySet()) {
     ...
     */
    public Set<Identifier> getAllLockedItems() {
     ...
     */
    public Map<Identifier, StageId> getAllItemLocks() {
    ...
    private Map<Identifier, StageId> resolvedItemLocksCache = null;
     ...
     */
    public Map<Identifier, StageId> getAllResolvedItemLocks() {
        ...
        Map<Identifier, StageId> resolved = new HashMap<>();
        ...
        if (!unlockedItems.isEmpty()) {
            ...
            for (Identifier unlocked : unlockedItems) {
     ...
     */
    public Set<Identifier> getAllLockedRecipes() {
     ...
     */
    public Map<Identifier, StageId> getAllRecipeLocks() {
     ...
     */
    public Set<Identifier> getUnlockedFluids() {
     ...
     */
    public boolean isFluidUnlocked(Identifier fluidId) {
     ...
     */
    public Set<Identifier> getUnlockedBlocks() {
     ...
     */
    public boolean isBlockUnlocked(Identifier blockId) {
     ...
     */
    public Map<Identifier, StageId> getAllFluidLocks() {
     ...
     */
    public Map<Identifier, StageId> getAllFluidTagLocks() {
     ...
     */
    public Optional<StageId> getRequiredStageForFluid(Identifier fluidId) {
        if (fluidId == null) {
            return Optional.empty();
        }
