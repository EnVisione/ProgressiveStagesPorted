...
public final class CraftingRecipeTracker {
    ...
    private static final Map<UUID, Identifier> LAST_RECIPE_BY_PLAYER = new ConcurrentHashMap<>();

    private CraftingRecipeTracker() {}

    /** Store the recipe ID that just matched the crafting grid for this player. */
    public static void setLastRecipe(UUID playerId, Identifier recipeId) {
        if (recipeId != null) {
    ...
    public static Identifier getLastRecipe(UUID playerId) {
