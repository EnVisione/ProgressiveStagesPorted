package com.enviouse.progressivestagesported.server.commands;
...
import com.enviouse.progressivestagesported.common.util.TextUtil;
import com.enviouse.progressivestagesported.server.loader.StageFileLoader;
import com.enviouse.progressivestagesported.server.triggers.TriggerPersistence;
...
public class StageCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        dispatcher.register(Commands.literal("progressivestagesported")
            .requires(Commands.hasPermission(new net.minecraft.server.permissions.PermissionCheck.Require(net.minecraft.server.permissions.Permissions.COMMANDS_ADMIN)))
        ...
        if (!missingDeps.isEmpty() && !StageConfig.isLinearProgression()) {
            ...
            if (confirmExpiry != null && now < confirmExpiry) {
                ...
                StageManager.getInstance().grantStageBypassDependencies(player, stageId, 
                    com.enviouse.progressivestagesported.common.api.StageCause.COMMAND);
    ...
    private static int reloadStages(CommandContext<CommandSourceStack> context) {
        ...
        com.enviouse.progressivestagesported.server.triggers.TriggerConfigLoader.reload();

        // Re-sync all online players with updated lock data and stage definitions
        var server = context.getSource().getServer();
        int syncedPlayers = 0;
        for (var player : server.getPlayerList().getPlayers()) {
            com.enviouse.progressivestagesported.common.network.NetworkHandler.sendStageDefinitionsSync(player);
            var stages = StageManager.getInstance().getStages(player);
            com.enviouse.progressivestagesported.common.network.NetworkHandler.sendStageSync(player, stages);
            com.enviouse.progressivestagesported.common.network.NetworkHandler.sendLockSync(player);
            syncedPlayers++;
        }
    ...
    private static int validateStages(CommandContext<CommandSourceStack> context) {
        ...
        List<String> startingStages = com.enviouse.progressivestagesported.common.config.StageConfig.getStartingStages();
    ...
    private static int ftbStatus(CommandContext<CommandSourceStack> context, ServerPlayer player) {
        context.getSource().sendSuccess(() -> Component.literal("FTB Quests integration is not available in this MC version.").withStyle(ChatFormatting.YELLOW), false);
        return 1;
    }
