...
public class ItemEnforcer {
    ...
    public static void playLockSound(ServerPlayer player) {
        try {
            ...
            Identifier soundLoc = Identifier.parse(soundStr);
            SoundEvent sound = BuiltInRegistries.SOUND_EVENT.getValue(soundLoc);

            if (sound != null) {
                player.level().playSound(null, player.getX(), player.getY(), player.getZ(), sound, SoundSource.PLAYERS, StageConfig.getLockSoundVolume(), StageConfig.getLockSoundPitch());
            } else {
                // Fallback to pling
                player.level().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.NOTE_BLOCK_PLING.value(), SoundSource.PLAYERS, 1.0f, 1.0f);
            }
        } catch (Exception e) {
            // Fallback
            player.level().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.NOTE_BLOCK_PLING.value(), SoundSource.PLAYERS, 1.0f, 1.0f);
